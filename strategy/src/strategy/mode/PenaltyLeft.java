...
public class PenaltyLeft extends AbstractMode
{
	...
	public PenaltyLeft(Commander commander)
	{
		super(commander);
	...
	public void update(World world)
	{
		...
		if (ControlGUI.shootingLeft) dirAngleMod = 22 - dirAngle; 
		else dirAngleMod = -158 - dirAngle;
		...
		if ( Math.abs( dirAngleMod ) > 120 ) turnSpeedDelta = 0.2;
		else if ( Math.abs( dirAngleMod ) > 60 ) turnSpeedDelta = 0.2;
		...
		if ( !kicked && ( System.currentTimeMillis() - startTime ) > 3000 ) {
			commander.kick();
			kicked = true;
		}
